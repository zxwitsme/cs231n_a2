...
def affine_forward(x, w, b):
    ...
    out = np.dot(x.reshape(num_inputs,-1),w)+b
    pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(N):
      for j in range(F):
        for l in range(HO):
          for m in range(WO):
            out[i,j,l,m]=np.sum(x_padded[i,:,l*s:HH+l*s, m*s:WW+m*s]*w[j]) + b[j]
    ...
    for i in range(N):
      for j in range(F):
        for l in range(HO):
          for m in range(WO):
            db[j] += dout[i,j,l,m]
            dw[j] += x_padded[i,:,l*s:HH+l*s,m*s:WW+m*s]*dout[i,j,l,m]
            dx_padded[i,:,l*s:HH+l*s,m*s:HH+m*s] += w[j]*dout[i,j,l,m]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(N):
      for j in range(C):
        for k in range(HO):
          for l in range(WO):
            out[i, j, k, l] = np.max(x[i,j,k*s:HH+k*s,l*s:WW+l*s])
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
      for j in range(C):
        for k in range(HO):
          for l in range(WO):
            temp = x[i, j, k*s:HH+k*s, l*s:WW+l*s]
            m = np.max(temp)
            dx[i, j, k*s:HH+k*s, l*s:WW+l*s] = (temp == m) * dout[i, j, k, l]
